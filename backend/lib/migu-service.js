...
async function getPlayableInfo(resource, preferLossless = false) {
  ...
  const payload = await requestJson(
    `http://app.c.nf.migu.cn/MIGUM2.0/strategy/listen-url/v2.4?${params.toString()}`,
    {
...
async function searchSongs({ keyword, pageNo = 1, pageSize = 20 }) {
  if (!keyword) {
  ...
  let payload = await runSearch(keyword);
  let result = payload.songResultData?.result || [];
  ...
  if (containsChinese) {
    ...
    if (looksBroken) {
      ...
      if (fallbackKeyword) {
        payload = await runSearch(fallbackKeyword);
        result = payload.songResultData?.result || [];
      }
    }
  }
  ...
  return {
    ...
    total: Number(payload.songResultData?.totalCount || result.length),
  };
}

async function getSongUrl({ cid, flac = '0' }) {
  const resource = await getResourceInfoByCid(cid);
  const playInfo = await getPlayableInfo(resource, Number(flac) === 1);
  return playInfo.url;
}

async function getLyric({ cid }) {
  const resource = await getResourceInfoByCid(cid);
  let playInfo = null;
...
async function getSongDetail({ cid, flac = '0' }) {
  const resource = await getResourceInfoByCid(cid);
  const playInfo = await getPlayableInfo(resource, Number(flac) === 1);
